...
void FlyLine::localCallBack(const geometry_msgs::PoseStampedConstPtr &msg)
{
    ...
    if(fabs(msg->pose.position.x - beginpoint_(0))      < AllowError_d
       && fabs(msg->pose.position.y - beginpoint_(1))   < AllowError_d
    ...
    {
        in_line = true;
        ROS_INFO("Begin to loiter!!!");
    }
    if(in_line == false && in_land == false)
    {
        ROS_INFO("Taking off to loiterpoint!!! ");


        beginpoint_(0) = msg->pose.position.x;
        beginpoint_(1) = msg->pose.position.y;
    ...
    if(in_line == true)
    {
        ...
        if(line_t < loiter_t)
        {
            ROS_INFO("Loiter 20 second in this point!!!");
...
//        }
    ...
    if(in_land == true)
    {
        ...
        endpoint_(0) = msg->pose.position.x;
        endpoint_(1) = msg->pose.position.y;
...
void FlyLine::init()
{
    ...
    l_nh_.param<double>("allland_t",allland_t, 10.0);
    l_nh_.param<double>("loiter_t",loiter_t, 20.0);

    l_nh_.param<double>("AllowError_d",AllowError_d, 0.25);
    ...
    local_sub_  = l_nh_.subscribe("/mavros/local_position/local",1000,&FlyLine::localCallBack, this);
    pos_sp_pub_ = l_nh_.advertise<geometry_msgs::PoseStamped>("/mavros/setpoint_position/local",1000);
//    att_sp_pub_ = l_nh_.advertise<geometry_msgs::PoseStamped>("/mavros/setpoint_attitude/attitude",100);


    ROS_INFO("ROS Topic Publish and Subscribe Initialization");
}
