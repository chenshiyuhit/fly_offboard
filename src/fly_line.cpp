...
void FlyLine::init()
{
    ...
    l_nh_.param<double>("allland_t",allland_t, 5.0);
    ...
    l_nh_.param<double>("endpoint/x",endpoint_(0), 3.0);
    l_nh_.param<double>("endpoint/y",endpoint_(1), 0.0);
    l_nh_.param<double>("endpoint/z",endpoint_(2), 1.5);

 //   local_sub_  = l_nh_.subscribe("/mavros/local_position/pose",1000,&FlyLine::localCallBack, this);
...
//    att_sp_pub_ = l_nh_.advertise<geometry_msgs::PoseStamped>("/mavros/setpoint_attitude/attitude",100);
    ...
    local_sub_  = l_nh_.subscribe("/firefly/ground_truth/pose",1000,&FlyLine::localCallBack, this);
